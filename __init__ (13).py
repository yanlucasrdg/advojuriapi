...
class LedgerTransacao(Base, UUIDMixin, TimestampMixin):
    ...
    tipo: Mapped[TipoTransacao] = mapped_column(
        Enum(
            TipoTransacao,
            name="tipo_transacao",
            # Sem isso, o SQLAlchemy usa o NOME do membro Python ("RECARGA")
            # em vez do VALOR ("recarga") ao gravar — e o tipo ENUM do
            # Postgres só aceita os valores minúsculos criados na migration.
            # Bug real que só aparece contra Postgres de verdade; testes
            # locais em SQLite não pegam porque SQLite não valida ENUM nativo.
